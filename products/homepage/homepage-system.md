# 🏛️ Fainders Design System Constitution & Specification

이 문서는 `products/homepage` 프로젝트의 디자인 시스템 구성, 모노레포 마이그레이션 규칙, 그리고 AI(Cursor/Claude)와 디자이너가 반드시 준수해야 할 **디자인 헌법(Design Constitution)**입니다.
...
## 2. 컴포넌트 아키텍처 및 폴더 구조
기존에 독립적으로 존재하던 컴포넌트들을 `@fai/ui` 공용 패키지와 연동하여 체계적으로 관리하며, 지정된 경로 규칙을 엄격히 준수합니다.

### 2.1 공용 UI 컴포넌트 (`packages/ui/components/`)
독립적으로 격리되어 모노레포 전역에서 재사용되는 핵심 UI 요소입니다.
- `Button`
- `NavigationBar`
- `Header`
- `Footer`
...
해당 홈페이지 프로덕트(특화 페이지) 내에서만 제한적으로 재사용되는 요소들입니다.
...
- **데이터 위치:**
  - 원시 수치 및 CSS 변수: `products/homepage/src/app/globals.css`
  - Tailwind 클래스 매핑: `products/homepage/tailwind.config.ts`

---

## 4. AI (Cursor / Claude) 작업 지침
AI가 새로운 컴포넌트를 생성하거나 수정할 때 반드시 지켜야 할 프로세스입니다.

1. **토큰 확인:** 코드를 짜기 전, 항상 `tailwind.config.ts`와 `src/app/globals.css`를 먼저 읽고, 사용 가능한 시맨틱 유틸·변수 목록을 파악할 것.
...
## 5. 커뮤니케이션
- UI에 필요한 새로운 컬러나 여백 수치가 발생하면, 디자이너가 직접 `globals.css`에 변수를 추가하고 `tailwind.config.ts`를 업데이트합니다. **AI는 임의로 새로운 규칙이나 예외 유틸리티를 만들지 마세요.**

---

## 6. Git 버전 관리 (Version Control)
...
레이아웃은 `root/foundation/grid.json`에 정의된 토큰을 기준으로 하며, `tailwind.config.ts`의 `screens` · `container`에 동기화되어 있습니다.
