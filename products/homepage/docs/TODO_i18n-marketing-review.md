...
## Phase 최종 — 마케팅팀 확정 검수본 반영 (2026-07-31)

Slack #prj_homepage 스레드(2026-07-13 시작, 7/31 완료 보고)에서 김성태·신혜영 님이 최종 검수한
[번역 스프레드시트](https://docs.google.com/spreadsheets/d/1E5HQJB_vSgdDEJ2je11AXMVdNGAvtW7wkf42-nIL5eU)를
`messages/{en,ja}.json`에 반영했다. 위 Phase 1~N의 "초벌" 문구 대부분이 이 확정본으로 교체됨
(en 214건 / ja 237건, 총 451건). 매칭 방법: ko.json 값과 스프레드시트 ko 열을 정확/퍼지(유사도) 매칭 후
스크린 스코프로 좁혀 오탐 방지, 매칭 근거는 `feat/i18n-marketing-review` 브랜치 커밋 참조.

**함께 수정한 기존 버그(번역과 무관, 검수 과정에서 발견):**
- `footer.addressValue` — ko/en 모두 우편번호가 `0662`(마지막 "8" 누락)로 잘려 있던 오탈자를 `06628`로 수정. 실서비스에 이미 노출 중이던 실데이터 오류.

**이번에 반영 보류 — 사용자 확인 필요 (자동 반영하지 않음):**
1. **`footer.policies.cctv` (영상정보처리기기 운영 관리방침)** — 스프레드시트 en 셀에 번역 대신 "이 부분은 제거"라는 지시문이 들어있음. 7/28 Slack 논의(super swift는 무인매장이 아니므로 불필요)와 맥락이 일치 — 항목 자체를 footer에서 제거할지 확정 필요.
2. **신규 유튜브 영상 2건 추정** — 스프레드시트에 "AI는 똑같이 생긴 다른 빵을 구별 할 수 있을까(ep2. 틀린그림찾기 아티제편)", "VCO 챌린지 [ep1. AI는 얼마나 빠를까?]" 제목이 있으나 현재 `videoOverrides`의 19개 videoId 중 어느 것과도 매칭되지 않음 — 실제 YouTube 영상 ID 확인 후 추가 필요.
3. **`M44Ig8m4BfU`/`6mdVuqYxWI8` 영상 설명 미반영** — 현재 코드에는 수치(⑴99.7%/⑵75%/⑶15%)·해시태그가 포함된 상세 설명이 있는데, 스프레드시트는 훨씬 짧은 버전만 제공 — 그대로 덮으면 정보 손실이라 보류.
4. **footer 전화번호 en 셀 깨짐** — 스프레드시트 footer 7행 en 값이 `-6160`(숫자 계산 오류로 보임). 코드상 전화번호는 `packages/ui/.../Footer.tsx`의 `VALUES.tel` 하드코딩 상수라 이번 반영과 무관하지만, 스프레드시트 원본 오류이니 예슬님께 확인 요청 권장.
5. **김성태 님 7/31 노트(원문)**: "① 의역한 것이 많아 배포 전 영문 텍스트가 반영된 mockup 확인 필요 ② About us People 표기는 전부 Team으로" — ②는 `about.people.title` 반영 완료. ①은 미해결 — **배포 전 김성태 님의 mockup 확인을 받을 것**.
6. **`products.visionCheckout.reviews.3`(Comma Store) ja 전용 orphan 데이터** — ko/en에는 없고 ja.json에만 존재하는 리뷰 항목 발견(이번 작업으로 생긴 것 아님, 기존 상태). 렌더링에는 영향 없으나(구조는 config 배열 길이 기준) 방치된 데이터라 정리 필요 시 확인.

**검증**: `pnpm build && pnpm test`(62/62) 통과, 키 동기화(ko=en=ja, 위 5번 제외 diff 0) 확인, `check-mobile-overflow.mjs` 21라우트 통과, Playwright로 ko/en/ja × 7페이지 로케일 오염(한글/가나 누출) 스윕 — LEAK 없음.
