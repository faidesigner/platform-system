 ...
 * Footer
 *
 * 참조: root/components/web/footer/Footer.tsx
 *
 * 토큰 매핑 (인라인 스타일 0개):
 *   배경           → bg-bg-200                (var(--color-bg-200))
 *   회사명         → text-body-l font-semibold text-text-basic-primary
 *   라벨           → text-body-s font-normal  text-text-basic-tertiary
 *   값/정책링크    → text-body-s font-normal  text-text-basic-secondary
 *   구분선(|)      → text-border-secondary    (var(--color-border-secondary))
 *   SNS 버튼 배경  → bg-filled-optional-brand-secondaryBtn
 *   SNS 버튼 패딩  → p-[var(--padding-XS)]
 *   라벨 min-width → min-w-[var(--size-100)]
 */
...
export default function Footer() {
  return (
    <footer className="relative w-full bg-bg-200 flex flex-col items-start px-6 py-10 gap-[40px] md:py-[56px] md:px-[150px] md:items-end">
      {/* 스크롤 버튼 — 우측 56px 고정 */}
      <div className="absolute top-[var(--size-56)] right-[var(--size-56)]">
        <ScrollTopButton />
      </div>
