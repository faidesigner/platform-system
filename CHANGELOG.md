...
#### VCO 히어로 영상 로케일별 교체 (HOM-70 — 2026-08-12부터 에셋 대기였던 건)

신고: "VCO 페이지 안내 동영상에 **한글 자막이 표시**됩니다. 일본어자막 버전 또는 자막이 없는 버전으로 교체해 주시면 좋을 것 같습니다"(2026-09-02 Hyeyoung Shin).

**결함 실증** — 프레임을 뜯어 확인했다.
- 기존 히어로(`vco-hero-bg.mp4`)는 t=12s에 `상품을 들고 VCO로 진입하면`이 **번인(burned-in)**돼 있다. 세 로케일이 이 파일 하나를 공유하고 있었으니 en·ja 방문자에게 한국어 자막이 그대로 나갔다

**반영**
| 로케일 | 파일 | 길이 | 용량 | 자막 |
|---|---|---|---|---|
| ko | `vco-hero-bg.mp4` | 66.6s | 6.2MB | 한국어(유지) |
| en | `vco-hero-bg-en.mp4` 🆕 | 66.6s | 5.7MB | **없음** · 영어 오프닝 |
| ja | `vco-hero-bg-ja.mp4` 🆕 | 26.6s | 2.8MB | **일본어**(`早く・正確に・簡単にお会計ができます` 확인) |

- 배선은 2026-08-12에 이미 끝나 있었다(`config/locale-policy.ts`의 `vcoHeroVideo`) — 상수 3개만 채웠다
- 압축: `scripts/optimize-videos.mjs`(1080p / H.264 CRF 28 / preset slow / `-an` / `+faststart`). **117.6MB → 5.7MB(-95%)**, **44.0MB → 2.8MB(-94%)**. 기존 ko(6.2MB)와 같은 급이라 로딩 부담이 늘지 않는다
- 검증: 세 로케일 `<video>`의 `currentSrc`·duration·1920×1080·muted·loop·autoplay 실측 + 자막 영역 프레임 크롭 대조(ko 한국어 / ja 일본어 / en 없음)

**원본을 커밋하지 않은 이유** — 이 저장소는 `*_original.mp4`를 함께 커밋하는 관례지만 이 둘은 제외했다.
- 히어로 배경에 원본 화질이 필요 없고, **en 원본은 117.6MB로 GitHub 파일당 100MB 하드 리밋을 넘어 push 자체가 거부된다**(현재 추적 중 최대 파일은 30.4MB)
- 원본의 단일 출처는 Notion HOM-70 카드의 Google Drive 링크다. 재인코딩이 필요하면 거기서 받아 같은 스크립트를 돌리면 된다 — `locale-policy.ts` 주석에 적어뒀다

**⚠️ 남은 확인 2건**
- **ja는 ko·en과 다른 편집본**이다(26.6s vs 66.6s — en은 ko와 밀리초까지 같은 컷이다). 히어로 루프 주기가 2.5배 짧아진다. 디자인 의도인지 확인이 필요하지만, 한국어 자막 노출을 없애는 쪽이 우선이라 먼저 연결했다
- **en 반영은 2026-09-03 이선연님 계획("이번 차시에는 영문 버전도 한글 자막으로 배포")과 다르다.** 그 판단의 전제는 "영문 자막이 없어 별도 번역 작업이 필요하다"였는데, 실제로 필요한 건 번역이 아니라 **이미 있는 자막 없는 파일을 쓰는 것**이었다(원 요청자가 허용한 "자막이 없는 버전"에 해당하고, ko와 같은 컷이라 그대로 대체된다). 되돌리려면 `locale-policy.ts` en 한 줄이면 된다
...
- `footer.japanEntity.emailLabel` / `emailValue` 신설(ja 전용) + `FooterBridge`의 `extraEntity.rows` 마지막 행으로 렌더. 데스크톱·모바일 양쪽 실측 확인

**이건 2026-08-28 HOM-101 결정의 일부를 되돌리는 변경이다 — 맥락을 남긴다.**
- HOM-101(같은 요청자)에서 ja 메일을 뺀 이유는 두 가지였다:
  ① 본사 이메일이 일본 법인 정보 **위**에 놓여 위계가 어색했다
  ② next-intl이 메시지 번들을 HTML script 페이로드로 직렬화하므로 키가 남으면 **주소가 페이지 소스에 그대로 실린다**(스팸 크롤러는 렌더 결과가 아니라 소스를 긁는다)
- **①은 해소된다** — 이번엔 일본 법인 블록 **안**이고, 노출 주소도 본사(contact@)가 아니라 일본 법인 전용(contact_jp@)이다
- **②는 해소되지 않는다.** 소스 노출은 이 결정의 알려진 대가이고, **사용자가 감수하기로 확인**했다(2026-09-03)
- **본사 이메일은 계속 ja 번들에서 제외** — HOM-101의 ①은 여전히 유효하다. 되돌리지 말 것

### 🧪 Tests
- `FooterBridge.test.tsx` HOM-101 관련 3건을 새 규칙으로 갱신. 테스트가 `contact_jp@`를 이름으로 박아 막고 있어서 그냥 통과시키지 않고 **규칙 이력(①최초→②08-28 삭제→③09-03 재노출)을 주석에 남기고** 조건을 바꿨다
  - ja에 **본사 주소(contact@)가 새어나오면 실패** — HOM-101에서 아직 유효한 부분을 계속 지킨다
  - ja 번들에 일본 법인 주소가 **있어야** 통과 + 본사 주소는 **없어야** 통과
  - 메일 행이 **소재지보다 뒤**에 오는지 순서까지 검사 — 배치가 이 규칙의 핵심이라서다
